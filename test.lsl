...
string Author = "Debbie Trilling"; string Supplier = "The Particle Crucible"; string ObjectName = "Particle Text Generator AV SE"; string Version = " v1.0";

integer TotalNoScansAllowed = 0;


InitialiseObject() { Power = FALSE; PowerText = "On"; ObjectOwner = llGetOwner(); llSetObjectName(ObjectName + Version); llSetObjectDesc("supplied courtesy of " + Author + "'s " + Supplier); vector PrimSize = llGetScale(); Radius = ((PrimSize.z / 2) + (FontSize/2)) + 0.50; AnnounceWelcome(); StopOperation(); }
...
StartSensorCheck() { if (FindRandomAV) { llSensor("",NULL_KEY, AGENT, Range, ScanArc); } else { ApplyDefaultText(); } }
